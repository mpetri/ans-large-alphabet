...
int main(int argc, char const* argv[])
{
    ...
    auto written_bytes = shuff_compress(output_u8.data(), output_u8.size(),
        input_u32s.data(), input_u32s.size());
    std::cout << "written_bytes = " << written_bytes << std::endl;

    std::cout << double(written_bytes * 8) / double(input_u32s.size()) << " BPI"
              << std::endl;

    std::vector<uint32_t> recover(input_u32s.size());
    shuff_decompress(
        recover.data(), input_u32s.size(), output_u8.data(), written_bytes);

    REQUIRE_EQUAL(
        input_u32s.data(), recover.data(), input_u32s.size(), "shuff");

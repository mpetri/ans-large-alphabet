...
std::pair<uint32_t, uint32_t> ans_fold_mapping(
    uint32_t x, uint32_t fidelity, uint32_t radix)
{
...
std::pair<uint32_t, uint32_t> ans_fold_undo_mapping(
    uint32_t x_plus_offset, uint32_t fidelity, uint32_t radix)
{
...
int main()
{
    ...
    for (uint32_t x = 0; x < std::numeric_limits<uint32_t>::max(); x++) {
        ...
        auto [unmapped, unmapped_bytes]
            = ans_fold_undo_mapping(mapped, fidelity, radix);
        auto masked_x = x >> (out_bytes * radix);
        masked_x = masked_x << (out_bytes * radix);
        if (masked_x != unmapped) {
            std::cout << "x = " << x << " out_bytes = " << out_bytes
                      << " masked_x = " << masked_x
                      << " unmapped = " << unmapped << std::endl;
        }
        if (out_bytes != prev
            || x == std::numeric_limits<uint32_t>::max() - 1) {
            std::cout << x << " -> " << mapped << " [" << out_bytes << "] ("
                      << unmapped_bytes << " output bytes)" << std::endl;
        }
        prev = out_bytes;
    }
}
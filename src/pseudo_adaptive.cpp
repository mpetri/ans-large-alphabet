...
void run(std::vector<std::vector<uint32_t>>& inputs,
    std::vector<std::string>& input_names, std::vector<size_t>& block_sizes)
{
    for (size_t i = 0; i < inputs.size(); i++) {
        ...
        for (auto block_size : block_sizes) {
            ...
            for (size_t j = 0; j < num_blocks; j++) {
                for (size_t k = 0; k < remapped_alphabet.size(); k++)
                    remapped_alphabet[k] = 0;

                for (size_t k = 0; k < block_size; k++) {
                    remapped_alphabet[in_ptr[k]] = 1;
                }
                if (remapped_alphabet[0] == 1)
                    block_alphabet.push_back(0);
                for (size_t k = 1; k <= max_sym; k++) {
                    if (remapped_alphabet[k] == 1)
                        block_alphabet.push_back(k);
                ...
                *enc_ptr_u32++ = block_alphabet.back() + 1;
                auto bytes_written = interpolative_internal::encode(enc_ptr_u32,
                    block_alphabet.data(), block_alphabet.size(),
                    block_alphabet.back() + 1);
                enc_ptr += (bytes_written + 8);
                enc_size -= (bytes_written + 8);

                if (block_alphabet.size() != 1) {
                    auto encoded_bytes
                        = t_compressor::encode(remapped_block_data.data(),
                            block_size, enc_ptr, enc_size, tmp_buf.data());
            ...
            if (last_block) {
                ...
                *enc_ptr_u32++ = block_alphabet.back() + 1;
                ...
                if (block_alphabet.size() != 1) {
                    auto encoded_bytes
                        = t_compressor::encode(remapped_block_data.data(),
                            last_block, enc_ptr, enc_size, tmp_buf.data());
                    total_enc_bytes += encoded_bytes;
                }
            }

            double BPI = double(total_enc_bytes * 8) / double(input.size());
            std::cout << input_name << ";" << t_compressor::name() << ";"
                      << block_size << ";" << BPI << std::endl;
        }
    }
}

...
void run(std::vector<uint32_t>& input, std::string input_name)
{
    ...
    // (4) output stats
    printf("%25.25s\t\t%15u\t\t%15u\t\t%18.18s\t\t%2.4f\t\t%2.4f\t\t%10.0f\t\t%"
           "10.0f\t\t\n",
        input_name.c_str(), (uint32_t)input.size(), (uint32_t)sigma,
        t_compressor::name().c_str(), input_entropy, BPI, encode_IPS,
        decode_IPS);
    fflush(stdout);
}
...
int main(int argc, char const* argv[])
{
    ...
    for (boost::filesystem::directory_iterator i(input_dir); i != end_itr;
        ...
        for (size_t k = 1; k <= 32; k++) {
            ...
            printf("%s;%u;%u;%2.4f\n", short_name.c_str(), (unsigned int)sigma,
                (unsigned int)k, input_entropy);
        }
    }

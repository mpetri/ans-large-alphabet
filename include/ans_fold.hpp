...
struct ans_fold_encode {
    ...
    void encode_symbol(uint64_t& state,uint32_t sym,uint8_t*& out_u8) {
        auto mapped_sym = ans_fold_mapping_and_exceptions<fidelity>(sym,out_u8);
    ...
    static ans_fold_decode load(const uint8_t* in_u8) {
        ...
        for(size_t sym=0;sym<=max_sym;sym++) {
            ...
            for(uint32_t k=0;k<cur_freq;k++) {
                ...
                model.table[cur_base+k].mapped_num = ans_fold_undo_mapping<fidelity>(sym) + (except_bytes << 30);
...
template<uint32_t fidelity>
size_t ans_fold_compress(uint8_t* dst, size_t dstCapacity,const uint32_t* src, size_t srcSize)
{
...
template<uint32_t fidelity>
void ans_fold_decompress(uint32_t* dst,  size_t to_decode,const uint8_t* cSrc, size_t cSrcSize)
{

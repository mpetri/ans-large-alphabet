...
template<uint32_t fidelity>
uint16_t ans_reorder_fold_mapping_and_exceptions(uint32_t x, uint8_t*& except_out)
{
    const uint32_t radix = 8;
    uint32_t radix_mask = ((1<<radix)-1);
    size_t offset = 0;
...
template<uint32_t fidelity>
struct ans_reorder_fold_encode {
    static ans_reorder_fold_encode create(const uint32_t* in_u32,size_t n) {
        ...
        std::vector<std::pair<int64_t,uint32_t>> unmapped_freqs(unmapped_max_sym+1);
        for(size_t i=0;i<n;i++) {
        ...
        uint32_t max_sym = 0;
        for(size_t i=0;i<n;i++) {
            auto mapped_u32 = ans_reorder_fold_mapping<fidelity>(model.mapping[in_u32[i]]);
            freqs[mapped_u32]++;
    ...
    void encode_symbol(uint64_t& state,uint32_t sym,uint8_t*& out_u8) {
        auto mapped_sym = ans_reorder_fold_mapping_and_exceptions<fidelity>(mapping[sym],out_u8);
...
uint32_t ans_reorder_fold_undo_mapping(const dec_entry_reorder_fold& entry, const uint8_t*& in_u8)
{
...
template<uint32_t fidelity>
uint32_t ans_reorder_fold_undo_mapping(std::vector<uint32_t>& most_frequent,uint32_t x_plus_offset)
{
    const uint32_t radix = 8;
...
template<uint32_t fidelity>
struct ans_reorder_fold_decode {
    ...
    static ans_reorder_fold_decode load(const uint8_t* in_u8) {
        ...
        for(size_t sym=0;sym<=max_sym;sym++) {
            ...
            uint32_t except_bytes = ans_reorder_fold_exception_bytes<fidelity>(sym);
            uint32_t mapped_num = ans_reorder_fold_undo_mapping<fidelity>(most_frequent,sym) + (except_bytes << 30);
...
template<uint32_t fidelity>
size_t ans_reorder_fold_compress(uint8_t* dst, size_t dstCapacity,const uint32_t* src, size_t srcSize)
{
...
template<uint32_t fidelity>
void ans_reorder_fold_decompress(uint32_t* dst,  size_t to_decode,const uint8_t* cSrc, size_t cSrcSize)
{

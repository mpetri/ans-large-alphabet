...
/* qsufsort.c
   ...
   Suffix Sorting" by N. Jesper Larsson (njlarsson@avadeaux.net) and Kunihiko
   Sadakane. Final version in Theoretical Computer Science Volume 387 Issue 3,
   November, 2007 Pages 258-272.
...
static int transform(int* x, int* p, int n, int k, int l, int q)
{
    ...
    if (d <= n) { /* if bucketing possible, compact alphabet.*/
        ...
        for (pi = x + r, c = b; pi <= x + n; ++pi) {
            ...
            c = (c & m) << s
                | (*pi - l + 1); /* shift in next old symbol in chunk.*/
        }
        ...
        for (pi = x, pj = x + r, c = b; pj <= x + n; ++pi, ++pj) {
            *pi = p[c]; /* transform to new alphabet.*/
            c = (c & m) << s
                | (*pj - l + 1); /* shift in next old symbol in chunk.*/
        }

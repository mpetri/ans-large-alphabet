...
struct ans_int_encode {
    static ans_int_encode create(const uint32_t* in_u32,size_t n) {
        ...
        for(size_t sym=0;sym<=max_sym;sym++) {
            if(model.nfreqs[sym] == 0) continue;
    ...
    std::vector<enc_entry_int> table;
    uint64_t frame_size;
    uint64_t lower_bound;
};

struct dec_entry_int {
    uint32_t freq;
    uint32_t offset;
    uint32_t sym;
};

struct dec_entry_int_small {
    uint16_t freq;
    uint16_t offset;
    uint32_t sym;
};
...
struct ans_int_decode {

    static ans_int_decode load(const uint8_t* in_u8) {
        ans_int_decode model;
        model.nfreqs = ans_load_interp(in_u8);
        auto max_norm_freq = *std::max_element(model.nfreqs.begin(),model.nfreqs.end());
        model.frame_size = std::accumulate(std::begin(model.nfreqs),std::end(model.nfreqs), 0);
        ...
        if(max_norm_freq <= std::numeric_limits<uint16_t>::max()) {
            model.table.resize(model.frame_size * sizeof(dec_entry_int_small));
            model.table_type = dec_table_type::SMALL;
            auto table = reinterpret_cast<dec_entry_int_small*>(model.table.data());
            for(size_t sym=0;sym<=max_sym;sym++) {
                auto cur_freq = model.nfreqs[sym];
                for(uint32_t k=0;k<cur_freq;k++) {
                    dec_entry_int_small* entry = table + cur_base + k;
                    entry->freq = cur_freq;
                    entry->sym = sym;
                    entry->offset = k;
                }
            ...
            model.table.resize(model.frame_size * sizeof(dec_entry_int));
            model.table_type = dec_table_type::LARGE;
            auto table = reinterpret_cast<dec_entry_int*>(model.table.data());
            for(size_t sym=0;sym<=max_sym;sym++) {
                auto cur_freq = model.nfreqs[sym];
                if(cur_freq == 0) continue;
                for(uint32_t k=0;k<cur_freq;k++) {
                    dec_entry_int* entry = table + cur_base + k;
                    table[cur_base+k].freq = cur_freq;
                    table[cur_base+k].sym = sym;
                    table[cur_base+k].offset = k;
                }
                dec_entry_int* entry = table + cur_base;
                cur_base += model.nfreqs[sym];
            }
        }
        model.lower_bound = constants::K * model.frame_size;
    ...
    uint64_t frame_log2;
    uint64_t lower_bound;
    dec_table_type table_type;
    std::vector<uint8_t> table;
};

size_t ans_int_compress(uint8_t* dst, size_t dstCapacity,const uint32_t* src, size_t srcSize)
{
...
void ans_int_decompress(uint32_t* dst,  size_t to_decode,const uint8_t* cSrc, size_t cSrcSize)
{
    const uint32_t num_states = 4;
    ...
    if( ans_frame.table_type == dec_table_type::SMALL) {
        while(cur_idx != fast_decode) {
            out_u32[cur_idx] = ans_frame.decode_sym<dec_entry_int_small>(states[0],in_u8);
            out_u32[cur_idx+1] = ans_frame.decode_sym<dec_entry_int_small>(states[1],in_u8);
            out_u32[cur_idx+2] = ans_frame.decode_sym<dec_entry_int_small>(states[2],in_u8);
            out_u32[cur_idx+3] = ans_frame.decode_sym<dec_entry_int_small>(states[3],in_u8);
            cur_idx += num_states;
        }
        while(cur_idx != to_decode) {
            out_u32[cur_idx++] = ans_frame.decode_sym<dec_entry_int_small>(states[num_states-1],in_u8);
        }
    } else {
        while(cur_idx != fast_decode) {
            out_u32[cur_idx] = ans_frame.decode_sym<dec_entry_int>(states[0],in_u8);
            out_u32[cur_idx+1] = ans_frame.decode_sym<dec_entry_int>(states[1],in_u8);
            out_u32[cur_idx+2] = ans_frame.decode_sym<dec_entry_int>(states[2],in_u8);
            out_u32[cur_idx+3] = ans_frame.decode_sym<dec_entry_int>(states[3],in_u8);
            cur_idx += num_states;
        }
        while(cur_idx != to_decode) {
            out_u32[cur_idx++] = ans_frame.decode_sym<dec_entry_int>(states[num_states-1],in_u8);
        }
    }
}
...
struct ans_msb_encode {
    ...
    void encode_symbol(uint64_t& state,uint32_t sym,uint8_t*& out_u8) {
        auto mapped_sym = ans_msb_mapping_and_exceptions(sym,out_u8);
    ...
    std::vector<enc_entry_msb> table;
    uint64_t frame_size;
    uint64_t lower_bound;
};

#pragma pack(push, 1)
struct dec_entry_msb {
    uint32_t freq;
    uint32_t offset;
    uint32_t mapped_num;
};
...
inline uint32_t ans_msb_undo_mapping(const dec_entry_msb& entry, const uint8_t*& in_u8)
{
...
uint32_t ans_msb_exception_bytes(uint32_t x)
{
    if (x <= 256)
        return 0;
    if (x <= 512)
...
struct ans_msb_decode {
    static ans_msb_decode load(const uint8_t* in_u8) {
        ...
        for(size_t sym=0;sym<=max_sym;sym++) {
            ...
            for(uint32_t k=0;k<cur_freq;k++) {
                ...
                model.table[cur_base+k].mapped_num = ans_msb_undo_mapping(sym) + (except_bytes << 30);
    ...
    uint64_t frame_log2;
    uint64_t lower_bound;
    std::vector<dec_entry_msb> table;
};


size_t ans_msb_compress(uint8_t* dst, size_t dstCapacity,const uint32_t* src, size_t srcSize)
{
...
void ans_msb_decompress(uint32_t* dst,  size_t to_decode,const uint8_t* cSrc, size_t cSrcSize)
{
    const uint32_t num_states = 4;

...
#define CATCH_INTERNAL_CONFIG_CPP11_NULLPTR
    ...
    && defined(CATCH_GCC_HAS_NEW_PRAGMA)
#define CATCH_INTERNAL_SUPPRESS_PARENTHESES_WARNINGS                           \
...
void throwLogicError(
    std::string const& message, SourceLineInfo const& locationInfo);
...
#define INTERNAL_CATCH_TESTCASE2(TestName, ...)                                \
    ...
    namespace {                                                                \
        Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(&TestName,    \
...
struct AssertionInfo {
    ...
    AssertionInfo(std::string const& _macroName,
        SourceLineInfo const& _lineInfo, std::string const& _capturedExpression,
...
namespace Matchers {
    namespace Impl {
        ...
        template <typename ObjectT, typename ComparatorT>
        MatchNotOf<ComparatorT> MatcherBase<ObjectT, ComparatorT>::operator!()
            const
        {
    ...
    template <typename T>
    inline Impl::MatchNotOf<T> Not(
        Impl::MatcherBase<T> const& underlyingMatcher)
    {
        return Impl::MatchNotOf<T>(underlyingMatcher);
    }
    template <typename T>
    inline Impl::MatchAllOf<T> AllOf(
        Impl::MatcherBase<T> const& m1, Impl::MatcherBase<T> const& m2)
    {
        return Impl::MatchAllOf<T>() && m1 && m2;
    }
    template <typename T>
    inline Impl::MatchAllOf<T> AllOf(Impl::MatcherBase<T> const& m1,
        Impl::MatcherBase<T> const& m2, Impl::MatcherBase<T> const& m3)
    {
        return Impl::MatchAllOf<T>() && m1 && m2 && m3;
    }
    template <typename T>
    inline Impl::MatchAnyOf<T> AnyOf(
        Impl::MatcherBase<T> const& m1, Impl::MatcherBase<T> const& m2)
    {
...
public:
    ...
    void captureMatch(
        ArgT const& arg, MatcherT const& matcher, char const* matcherString);
...
// nullptr_t support based on pull request #154 from Konstantin Baumann
    ...
    // So the compare overloads can be operator agnostic we convey the operator
    // as a template enum, which is used to specialise an Evaluator for doing
    // the comparison.
...
inline void ResultBuilder::captureMatch(
    ArgT const& arg, MatcherT const& matcher, char const* matcherString)
{
    MatchExpression<ArgT const&, MatcherT const&> expr(
        arg, matcher, matcherString);
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                \
    do {                                                                       \
        ...
        && static_cast<bool>(                                                  \
               !!(expr)))) // expr here is never evaluated at runtime but it
...
#define INTERNAL_CATCH_THROWS_AS(                                              \
    ...
    do {                                                                       \
        Catch::ResultBuilder __catchResult(macroName, CATCH_INTERNAL_LINEINFO, \
            #expr ", " #exceptionType, resultDisposition);                     \
        if (__catchResult.allowThrows())                                       \
            try {                                                              \
                static_cast<void>(expr);                                       \
                __catchResult.captureResult(                                   \
                    Catch::ResultWas::DidntThrowException);                    \
            } catch (Catch::add_const<                                         \
                Catch::add_lvalue_reference<exceptionType>::type>::type) {     \
                __catchResult.captureResult(Catch::ResultWas::Ok);             \
            } catch (...) {                                                    \
                __catchResult.useActiveException(resultDisposition);           \
            }                                                                  \
...
struct IMutableRegistryHub {
    ...
    virtual void registerReporter(
        std::string const& name, Ptr<IReporterFactory> const& factory)
        = 0;
...
namespace Detail {
    ...
    public:
        ...
        friend bool operator==(const T& lhs, Approx const& rhs)
        {
            ...
            bool relativeOK = std::fabs(lhs_v - rhs.m_value) < rhs.m_epsilon
                    * (rhs.m_scale
                          + (std::max)(
        ...
        {
            // Thanks to Richard Harris for his help refining this formula
            bool relativeOK = std::fabs(lhs - rhs.m_value) < rhs.m_epsilon
                    * (rhs.m_scale
        ...
        struct ContainsMatcher : MatcherBase<std::vector<T>, std::vector<T>> {
            ...
            bool match(std::vector<T> const& v) const CATCH_OVERRIDE
            {
                ...
                for (size_t i = 0; i < m_comparator.size(); ++i)
                    if (std::find(v.begin(), v.end(), m_comparator[i])
                        == v.end())
                        return false;
                return true;
            }
...
struct TestCaseInfo {
    ...
    friend void setTags(
        TestCaseInfo& testCaseInfo, std::set<std::string> const& tags);
...
const unsigned int consoleWidth = 80;
    ...
    struct TextAttributes {
        ...
        std::size_t width; // maximum width of text, including indent. Longer
                           // text will wrap
        char tabChar; // If this char is seen the indent is changed to current
                      // pos
    };
    ...
    public:
        ...
        void spliceLine(
            std::size_t _indent, std::string& _remainder, std::size_t _pos)
        {
            lines.push_back(
                std::string(_indent, ' ') + _remainder.substr(0, _pos));
...
namespace Detail {
    ...
    template <typename T> void convertInto(std::string const& _source, T& _dest)
    {
        ...
        if (ss.fail())
            throw std::runtime_error(
                "Unable to convert " + _source + " to destination type");
    }
    ...
    inline void convertInto(std::string const& _source, bool& _dest)
    {
        ...
        std::transform(
            sourceLC.begin(), sourceLC.end(), sourceLC.begin(), toLowerCh);
...
public:
    ...
    void parseIntoTokens(
        std::vector<std::string> const& args, std::vector<Token>& tokens)
    {
...
public:
    ...
    std::vector<Parser::Token> parseInto(
        std::vector<std::string> const& args, ConfigT& config) const
    {
            ...
            while (it != strEnd) {
                ...
                if (itEnd != strEnd) {
                    ...
                    if (!foundWrapPoint) {
                        ...
                        while (itEnd > it
                            && wrappableInsteadOfChars.find(*(itEnd - 1))
                                != std::string::npos)
                            --itEnd;
                    }
                }
...
namespace TestCaseTracking {
    ...
    public:
        ...
        virtual ITracker* findChild(
            NameAndLocation const& nameAndLocation) CATCH_OVERRIDE
        {
            Children::const_iterator it = std::find_if(m_children.begin(),
                m_children.end(), TrackerHasName(nameAndLocation));
    ...
    public:
            ...
            TrackerContext& ctx, NameAndLocation const& nameAndLocation)
        {
            ...
            if (ITracker* childTracker
                ...
                section
                    = new SectionTracker(nameAndLocation, ctx, &currentTracker);
        ...
        void addInitialFilters(std::vector<std::string> const& filters)
        {
            if (!filters.empty()) {
                ...
                std::copy(filters.begin(), filters.end(),
        ...
        {
            if (filters.size() > 1)
                std::copy(filters.begin() + 1, filters.end(),
                    std::back_inserter(m_filters));
        }
    };
    ...
    Totals runTest(TestCase const& testCase)
    {
        ...
        do {
            ...
            do {
                ...
                !m_testCaseTracker->isSuccessfullyCompleted() && !aborting());
        }
        ...
        try {
            m_lastAssertionInfo
                = AssertionInfo("TEST_CASE", testCaseInfo.lineInfo,
                    std::string(), ResultDisposition::Normal);
...
private:
    ...
    void handleUnfinishedSections()
    {
        ...
        for (std::vector<SectionEndInfo>::const_reverse_iterator it
...
void registerTestCase(ITestCase* testCase,
    char const* classOrQualifiedMethodName, NameAndDesc const& nameAndDesc,
    SourceLineInfo const& lineInfo)
{
...
void TestCase::swap(TestCase& other)
{
    ...
    std::swap(
        TestCaseInfo::properties, static_cast<TestCaseInfo&>(other).properties);
...
struct IReporter : IShared {
    ...
    virtual void StartSection(
        std::string const& sectionName, std::string const& description)
        = 0;
    virtual void EndSection(
        std::string const& sectionName, Counts const& assertions)
        = 0;
...
bool LegacyReporterAdapter::assertionEnded(AssertionStats const& assertionStats)
{
    if (assertionStats.assertionResult.getResultType() != ResultWas::Ok) {
        for (std::vector<MessageInfo>::const_iterator it
             = assertionStats.infoMessages.begin(),
...
void LegacyReporterAdapter::sectionEnded(SectionStats const& sectionStats)
{
    ...
    m_legacyReporter->EndSection(
        sectionStats.sectionInfo.name, sectionStats.assertions);
}
...
void LegacyReporterAdapter::testGroupEnded(TestGroupStats const& testGroupStats)
{
    ...
    m_legacyReporter->EndGroup(
        testGroupStats.groupInfo.name, testGroupStats.totals);
}
...
SectionInfo::SectionInfo(SourceLineInfo const& _lineInfo,
    std::string const& _name, std::string const& _description)
    : name(_name)
    , description(_description)
    , lineInfo(_lineInfo)
{
...
ResultBuilder::ResultBuilder(char const* macroName,
    SourceLineInfo const& lineInfo, char const* capturedExpression,
    ResultDisposition::Flags resultDisposition, char const* secondArg)
...
struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
    ...
    virtual void testGroupEnded(
        TestGroupStats const& /* _testGroupStats */) CATCH_OVERRIDE
    {
...
public:
    ...
    void writeGroup(TestGroupNode const& groupNode, double suiteTime)
    {
        ...
        // Write test cases
        for (TestGroupNode::ChildNodes::const_iterator it
        ...
        SectionNode const& sectionNode)
    {
        ...
        for (SectionNode::ChildSections::const_iterator it
    ...
    {
        for (SectionNode::Assertions::const_iterator it
...
struct ConsoleReporter : StreamingReporterBase {
    ...
    virtual bool assertionEnded(
        AssertionStats const& _assertionStats) CATCH_OVERRIDE
    {
...
#define CATCH_CAPTURE(msg)                                                     \
    INTERNAL_CATCH_INFO(#msg " := " << Catch::toString(msg), "CATCH_CAPTURE")
...
#define CATCH_FAIL_CHECK(...)                                                  \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure,                      \
        Catch::ResultDisposition::ContinueOnFailure, "CATCH_FAIL_CHECK",       \
        __VA_ARGS__)
...
#define CATCH_FAIL_CHECK(msg)                                                  \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure,                      \
        Catch::ResultDisposition::ContinueOnFailure, "CATCH_FAIL_CHECK", msg)
...
#define REQUIRE_THROWS_AS(expr, exceptionType)                                 \
    INTERNAL_CATCH_THROWS_AS(expr, exceptionType,                              \
        Catch::ResultDisposition::Normal, "REQUIRE_THROWS_AS")
...
#define CAPTURE(msg)                                                           \
    INTERNAL_CATCH_INFO(#msg " := " << Catch::toString(msg), "CAPTURE")
...
#define FAIL_CHECK(...)                                                        \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure,                      \
        Catch::ResultDisposition::ContinueOnFailure, "FAIL_CHECK",             \
        __VA_ARGS__)
#define SUCCEED(...)                                                           \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok,                                   \
...
#define FAIL_CHECK(msg)                                                        \
    INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure,                      \
        Catch::ResultDisposition::ContinueOnFailure, "FAIL_CHECK", msg)
#define SUCCEED(msg)                                                           \
    INTERNAL_CATCH_MSG(Catch::ResultWas::Ok,                                   \
